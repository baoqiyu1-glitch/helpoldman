<template>
  <view class="apply-container">
    ...
    <view class="page-header" v-if="!loading && !error">
      <text class="title">{{ textConfig.pageTitle || '志愿者申请' }}</text>
    </view>
    ...
    <view class="form-container" v-if="!loading && !error">
      ...
      <button type="primary" @click="submitApplication">{{ textConfig.submitBtnText || '提交申请' }}</button>
    </view>
  </view>
</template>
  ...
  methods: {
    ...
    async loadApplyInfo() {
      try {
        ...
        const response = await uni.request({
          url: 'http://localhost:8080/api/volunteer/apply-info',
          method: 'GET',
          header: {
        ...
        if (response[1].statusCode === 200 && response[1].data.code === 200) {
          this.applyInfo = response[1].data.data || {};
          // 如果已有申请信息，填充到表单
          if (this.applyInfo.status === '已认证') {
            this.form.name = this.applyInfo.name || '';
            this.form.age = this.applyInfo.age || '';
            this.form.phone = this.applyInfo.phone || '';
            this.form.email = this.applyInfo.email || '';
            this.form.address = this.applyInfo.address || '';
            this.form.availableTime = this.applyInfo.availableTime || '';
            this.form.introduction = this.applyInfo.introduction || '';
          }
        }
      } catch (error) {
  ...
  try {
    ...
    async submitApplication() {
      ...
      try {
        const token = uni.getStorageSync('token');
        const response = await uni.request({
          url: 'http://localhost:8080/api/volunteer/apply',
          method: 'POST',
          header: {
            'Authorization': token ? `Bearer ${token}` : '',
        ...
        if (response[1].statusCode === 200 && response[1].data.code === 200) {
          uni.showToast({
            title: this.textConfig.submitSuccessText || '申请提交成功',
            icon: 'success'
          });
          ...
          throw new Error(response[1].data.msg || '提交申请失败');
        }
      } catch (error) {
